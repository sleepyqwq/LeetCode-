...
**处理左括号**：如果遇到 `(`、`{` 或 `[`，说明有一个新的嵌套层级开始，将其压入栈中。

**处理右括号**：如果遇到 `)`、`}` 或 `]`，则需要判断：

- 如果此时栈为空，说明右括号多余，直接返回 `false`。
- 如果栈顶元素（最近一个未闭合的左括号）与当前右括号类型不匹配，说明顺序错误或类型不符，返回 `false`。
- 如果匹配成功，则将栈顶元素弹出，继续处理下一个字符。

**最终检查**：遍历结束后，如果栈为空，说明所有左括号都得到了闭合，返回 `true`；否则说明有残留的左括号，返回 `false`
...
class MinStack {
private:
    stack<int> st;     // 正常栈，存所有元素
    stack<int> minSt;  // 最小栈，存当前状态下的最小值
